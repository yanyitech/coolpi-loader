...
static int part_check_efi(struct blk_desc *dev_desc)
{
	ALLOC_CACHE_ALIGN_BUFFER_PAD(gpt_header, gpt_head, 1, dev_desc->blksz);
	gpt_entry *gpt_pte = NULL;
	int i = 0;
	...
	for (i = 0; i < le32_to_cpu(gpt_head->num_partition_entries); i++) {
		...
		ret = strcmp(print_efiname(&gpt_pte[1]), "3588boot");
		if( ret == 0) {
			//printf("match boot partition\n");
			break;
		}
	}
...
int check_mmc_part(int devnum)
{
	struct blk_desc *mmc_dev;
	int ret = 0;

	mmc_dev = blk_get_devnum_by_type(IF_TYPE_MMC, devnum);
	if (mmc_dev != NULL && mmc_dev->type != DEV_TYPE_UNKNOWN) {
			ret = part_check_efi(mmc_dev);
			return ret;
	}
